...
process.env.trendlynecookie='_ga=GA1.2.775644955.1603113261; __utma=185246956.775644955.1603113261.1614010114.1614018734.3; _gid=GA1.2.726050934.1646303470; csrftoken=g1ix3m675oZH0jMLS8Cq1ElNOZv6uiCuqtYqWtkEZlKWa24RHGFJjGae4DIrXfeD; .trendlyne=gtmzuecgks3q4zxl0luvj30kb90gm00r; _gat=1'
...
// const client1 = new MongoClient1(uri, { useNewUrlParser: true, useUnifiedTopology: true });
// client1.connect(err => {
//   const collection = client1.db("jdata").collection("allinone");
...
// bot.sendMessage(msg.chat.id, "Hello dear Amit");
// }
...
// }
// var Advay = "Advay";
// if (msg.text.toString().toLowerCase().includes(Advay)); {
// bot.sendPhoto(chatId, 'E:/Advay/IMG_20200309_181853.jpg');
// bot.sendMessage(msg.chat.id, "Hope to see you around again , Bye");
// }
...
// // bot.sendPhoto(chatId, 'E:/Advay/IMG_20200309_181853.jpg');
// //bot.sendPhoto(chatId, 'C:/Users/Amit/Downloads/my-image-name (4).jpeg'); {
...
// // });
//////////////////////////////////////Local Mongo Database Read Functions////////////////////////////////////////////
...
////////////////////////////////////////////////BLOOMBERG QUINT POST REQUESTS/////////////////////////////////////////


app.post('/bqbasicdetails', async function (req, res) {

  let bqsymbol = req.body

  //console.log(req.body)
  const promises = bqsymbol.map(symbol => {
    axios.get('https://www.bloombergquint.com/next/feapi/stock/' + symbol.bqsymbol + '/stock-dna').then((response) => {

      var obj1 = ({ PEratio: response.data.data.peRatio, Stock: symbol.name })

      // (response.data.data.sentiments.indication)
      //console.log(obj1.sentiments)
      //var json1 = (obj1.sentiments.indication)
      //var json2 = (JSON.parse(response.body).data)
      //json1 = json1.concat(json2);
      //var obj2 = JSON.parse(response.body).data
      //console.log(obj2.SC_FULLNM)
...
//////////////////////////////////////////////// Inserting stock into Watchlist ///////////////////////////////////////
app.post('/watchlistinsert', async function (req, res) {
...
//////////////////////////////////////////////// Deletting stock from Watchlist ///////////////////////////////////////////
app.post('/watchlistdelete', async function (req, res) {
...
////////////////////////////////////////////////////  Watchlist Stock view,in stocklist Component///////////////////////
...
//////////////////////////////////////////////////////PORTFOLIO////////////////////////////////////////////////////////
////////////////////////////////// Stocks added in Portfolio of Amit Kapila (Kotak Securities) view
...
//
/////////////////////////POST Requests which Bring data to Database///////////////////////////////////////////////////////////

//##################MONEY CONTROL#######################

//1.Money control Stock Daily/Weekly Monthly data,used in Dashboard Component
app.post('/mccombine', async function (req, res) {
  ...
  const promises = mcsymbol.map(symbol => {
    axios.all([
      axios.get('https://priceapi.moneycontrol.com/pricefeed/techindicator/D/' + symbol.mcsymbol + '?fields=sentiments'),
      axios.get('https://priceapi.moneycontrol.com/pricefeed/techindicator/W/' + symbol.mcsymbol + '?fields=sentiments'),
      axios.get('https://priceapi.moneycontrol.com/pricefeed/techindicator/M/' + symbol.mcsymbol + '?fields=sentiments')
...
//////////////////////////////////////////Moneycontrol Volume Post request////////////////////////////////////////////////
app.post('/mcvolume', async function (req, res) {
...
//////////////////////////////////////////Moneycontrol Insights Post request////////////////////////////////////////////////
app.post('/mcinsight', async function (req, res) {
  ...
  const promises = mcsymbol.map(symbol => {
    ...
    axios.get('https://api.moneycontrol.com//mcapi//v1//extdata//mc-insights?scId=' + symbol.mcsymbol+'&type=d').then((response) => {
     var obj1 = ({ Date: symbol.Date,Time:symbol.time, Name: symbol.name,Symbol:symbol.mcsymbol, "priceInsight": response.data.data.insightData.price[3].shortDesc })
...
///////////////////////////////////MoneyControl Post to get Technical Indicators in Mongo DB///////////////////////////////////
app.post('/moneycontrolti', async function (req, res) {
  ...
  var obj1 = [];
  const promises = mcsymbol.map(symbol => {
    axios.get('https://priceapi.moneycontrol.com/pricefeed/techindicator/D/' + symbol.mcsymbol + '?field=RSI').then((response) => {



      data = response.data.data
      data2 = data["indicators"]
      for (let i in data2) {
        if (data2[i]["id"] == null) {
          //console.log(obj1)
          //console.log(symbol.name + "is having null value")
        }
        
        else {
          obj1.push({ indicator: data2[i]["displayName"], id: data2[i]["id"], value: data2[i]["value"], isin: symbol.isin, stock: symbol.name });
...
///////////////////////////////////MoneyControl Post to get Stock Daily in Mongo DB///////////////////////////////////
app.post('/moneycontroloveralldaily', async function (req, res) {

  let mcsymbol = req.body

  //console.log(req.body)
  const promises = mcsymbol.map(symbol => {
    axios.get('https://priceapi.moneycontrol.com/pricefeed/techindicator/D/' + symbol.mcsymbol + '?fields=sentiments,pivotLevels,sma,ema').then((response) => {
...
/////////////////////////////////1.Money control Sector Daily/Weekly/Monthly data,used in Dashboard Component/Sector Component
app.post('/mcsectorcombine', async function (req, res) {

  let mcsectorsymbol = req.body

  //console.log(mcsectorsymbol)
  const promises = mcsectorsymbol.map(symbol => {
    axios.all([
      axios.get('https://priceapi.moneycontrol.com/pricefeed/techindicator/D/'+symbol.mcsectorsymbol+'?field=RSI'),
      axios.get('https://priceapi.moneycontrol.com/pricefeed/techindicator/W/'+symbol.mcsectorsymbol+'?field=RSI'),
      axios.get('https://priceapi.moneycontrol.com/pricefeed/techindicator/M/'+symbol.mcsectorsymbol+'?field=RSI')
    ]).then((responseArr) => {
...
///////////////////////2..Money control Stock Current Price Percentage increase data,used in Dashboard Component

app.post('/moneycontroloverall', async function (req, res) {
...
/////////////////////////////////////////3.Money control Stock P Score,used in Dashboard Component

app.post('/pscore', async function (req, res) {
  ...
  const promises = mcsymbol.map(symbol => {
    axios.get('https://api.moneycontrol.com/mcapi/v1/extdata/mc-insights?scId=' + symbol.mcsymbol + '&type=d').then((response) => {
      ...
      //   var dbo = db.db("moneycontrol");
      //   dbo.collection("pscore").insertOne(obj1, function(error, response) {
      //     if(error) {
      ...
      console.log("This is Outer loope error",error)
    })
...
//////////////////////////////////4.Money control moving average data post request to get moving averages

  app.post('/mcmovingaverages', async function (req, res) {

    let mcsymbol = req.body
    //console.log(req.body)
    const promises = mcsymbol.map(symbol => {
      axios.get('https://priceapi.moneycontrol.com/pricefeed/techindicator/D/' + symbol.mcsymbol + '?field=RSI').then((response) => {
     // console.log(response.data.data.sma)
         var obj3 = ({ indic: response.data.data.crossover, isin: symbol.isin, name: symbol.name ,Date:symbol.Date})
        var obj1 = [];
        //var obj1 = ({ isin: response.data.bseNseJson.isin,companyid:symbol.companyid })
        data = response.data.data
        data2 = data["crossover"]
         for (let i in data2) {
        //  // console.log(data2[i].key,data2[i].value,data2[i].indication)
           obj1.push({ Key:data2[i].key,value:data2[i].displayValue,indication:data2[i].indication,period:data2[i].period, stock: symbol.name,isin:symbol.isin,Date:symbol.Date });
         }
        // var obj2 = [];
        // data3 = data["ema"]
        // for (let i in data3) {
        //   obj1.push({ EMA:data3[i].key,Evalue:data3[i].value,Eindication:data3[i].indication, stock: symbol.name,isin:symbol.isin,Date:symbol.Date });
...
///////////////////////////////////////Moneycontrol read from DB for Daily Bullish//////////////////////////////////////////
...
///////////////////////////////////Moneycontrol get request to get data from DB for moving averages////////////
app.get('/mcmovingaveragesview', (req, res) => {
  let stockisin = req.query.stockisin
...
//1.Money control Stock Weekly ,used in Dashboard Component,can be removed
app.post('/moneycontroloverallweekly', async function (req, res) {

  let mcsymbol = req.body
  //console.log(req.body)
  const promises = mcsymbol.map(symbol => {
    axios.get('https://priceapi.moneycontrol.com/pricefeed/techindicator/W/' + symbol.mcsymbol + '?fields=sentiments,pivotLevels,sma,ema').then((response) => {
...
//     instancetrendlyne.get('https://trendlyne.com')
//       .then(data => instancetrendlyne.get('https://trendlyne.com/mapp/v1/stock/web/ohlc/' + symbol.tlid))
//       .then(data => {
//         if (data.data.body.stockData) {
//           MongoClient.connect(url, function (err, db) {
//             if (err) console.log(err)
//             var dbname = "trendlyne"
//             var dbo = db.db(dbname);
//             var obj = ({ name: symbol.name, symbol: symbol.tlid, DurabilityScore: data.data.body.stockData[6], VolatilityScore: data.data.body.stockData[7], MomentumScore: data.data.body.stockData[8] })
//             dbo.collection("score").insertOne(obj)
//             console.log("Object inserted in trendlyne/score")
//             db.close();
//           })
             
        
//         } else { console.log("No data present for " + symbol.name) }
//   //    .catch(data => console.error("There is error" + symbol.name))
...
// app.get('/kitestockreports', function (req, res) {
//   let timeframe = req.query.timeframe
//   let eqsymbol = req.query.eqsymbol
//   console.log(timeframe,eqsymbol)
